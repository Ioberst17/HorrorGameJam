...
public class InventoryManager : MonoBehaviour
{
    ...
    //    else
    //    {
    //        if (primaryWeapons.Count() > 0) { indexInPrimary = primaryWeapons.FindIndex(gameObject => Equals(weapon.id, gameObject.id)); } // returns -1 if object isn't indexed}
    //    }

    //    if (secondaryWeapons == null) { Debug.Log("Secondary Weapons Inventory list is null - check if initialized properly"); }
    //    else 
    //    {
    //        if(secondaryWeapons.Count() > 0) { indexInSecondary = secondaryWeapons.FindIndex(gameObject => Equals(weapon.id, gameObject.id));}
    //    }
    ...
    public string GetCurrentSecondaryWeapon() { return secondaryWeapons[currentSecondaryWeaponIndex].name; }
    ...
    bool CheckIfItemIsAmmoOrInstantUse(int itemID, int amount)
    {
        ...
        for(int i = 0; i < consumablesDB.Length; i++)
        {
            if(itemID == consumablesDB[i].id)
            {
                ...
                {
                    if(secondaryWeapons[secondaryWeaponsManager.currentWeaponIndex].name == consumablesDB[i].itemName) { secondaryWeaponsManager.AddAmmo(amount); }
                    else
                    {
